...
const categories = ["Pembangunan Perisian","Pembangunan Aplikasi","Kejuruteraan Produk Digital","Sistem Perniagaan","Persepaduan Sistem","AI & Automasi","Pengkomputeran Awan & Persepaduan API"];
const readiness = [
  ...
  ["Pengesahan kewangan", "Persendirian", "Pengesahan bank dan maklumat pendaftaran berkaitan hanya dikongsi melalui saluran persendirian yang diluluskan."],
  ["Semakan keselamatan & undang-undang", "Disokong", "NDA, perlindungan data dan soal selidik keselamatan boleh dilengkapkan apabila diperlukan oleh pelanggan."],
];
const documents = [
...
export default function VendorMalayPage() {
  return <main>
    ...
    <section className={`${styles.hero} shell`}><div className="eyebrow">Perolehan & Pendaftaran Vendor</div><h1>Maklumat pendaftaran vendor.</h1><p>Maklumat korporat utama untuk pasukan perolehan yang menilai atau mendaftarkan UNIRAS SDN. BHD. sebagai vendor teknologi. Dokumen sensitif dibekalkan melalui saluran persendirian yang sesuai dan tidak diterbitkan di laman web awam.</p></section>
    <section className={`${styles.section} shell`}><div className="section-kicker">Maklumat korporat</div><div className={styles.facts}><div><span>Nama sah syarikat</span><strong>UNIRAS SDN. BHD.</strong></div><div><span>No. pendaftaran</span><strong>1478108T / 202201032411</strong></div><div><span>Entiti</span><strong>Syarikat sendirian berhad</strong></div><div><span>Negara</span><strong>Malaysia</strong></div><div><span>Perniagaan utama</span><strong>Produk digital, aplikasi & sistem perniagaan</strong></div><div><span>Hubungan umum</span><strong>uniras.capital@gmail.com</strong></div></div></section>
    <section className={`${styles.section} ${styles.soft}`}><div className="shell"><div className="section-kicker">Kesediaan perolehan</div><div className={styles.heading}><h2>Maklumat awam yang jelas. Dokumen persendirian yang terkawal.</h2><p>Pasukan perolehan boleh menilai kesesuaian vendor dengan cekap sementara maklumat korporat, perbankan dan peribadi yang sensitif kekal dilindungi.</p></div><div className={styles.readiness}>{readiness.map(([title,status,body])=><article key={title}><div><span>{status}</span></div><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
