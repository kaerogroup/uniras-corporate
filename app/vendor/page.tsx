...
const categories = ["Software Development","Application Development","Digital Product Engineering","Business Systems","Systems Integration","AI & Automation","Cloud & API Integration"];
const readiness = [
  ["Public corporate information", "Ready", "Legal name, company registration number, entity type, country and technology service categories are published here."],
...
export default function VendorPage() {
  return <main>
    ...
    <section className={`${styles.section} ${styles.soft}`}><div className="shell"><div className="section-kicker">Procurement readiness</div><div className={styles.heading}><h2>Clear public information. Controlled private documents.</h2><p>Procurement teams can evaluate vendor fit efficiently while sensitive corporate, banking and personal information remains protected.</p></div><div className={styles.readiness}>{readiness.map(([title,status,body])=><article key={title}><div><span>{status}</span></div><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
