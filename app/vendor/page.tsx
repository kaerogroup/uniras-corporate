...
const readiness = [
  ["Public corporate facts", "Ready", "Legal name, company registration number, entity type, country and technology service categories are published here."],
  ["Corporate due diligence", "On request", "SSM profile and other supporting corporate documents can be supplied to an authorised procurement contact."],
  ["Financial verification", "Private", "Bank confirmation and related onboarding information are shared only through an approved private channel."],
  ["Security & legal review", "Supported", "NDA, data-protection and security questionnaires can be completed when required by the client."],
];
const documents = [
  ["SSM company profile / registration documents", "Available upon request"],
  ["Registered address and corporate particulars", "Provided with onboarding documents"],
  ["Tax and statutory information", "Available upon request, where applicable"],
  ["Bank account confirmation", "Shared securely for approved onboarding only"],
...
export default function VendorPage() {
  return <main>
    ...
    <section className={`${styles.section} shell`}><div className="section-kicker">Corporate information</div><div className={styles.facts}><div><span>Legal name</span><strong>UNIRAS SDN. BHD.</strong></div><div><span>Registration no.</span><strong>1478108T / 202201032411</strong></div><div><span>Entity</span><strong>Private limited company</strong></div><div><span>Country</span><strong>Malaysia</strong></div><div><span>Primary business</span><strong>Digital products, applications & business systems</strong></div><div><span>General contact</span><strong>uniras.capital@gmail.com</strong></div></div></section>
    <section className={`${styles.section} ${styles.soft}`}><div className="shell"><div className="section-kicker">Procurement readiness</div><div className={styles.heading}><h2>Clear public facts. Controlled private documents.</h2><p>Procurement teams can evaluate our vendor fit quickly while sensitive corporate, banking and personal information remains protected.</p></div><div className={styles.readiness}>{readiness.map(([title,status,body])=><article key={title}><div><span>{status}</span></div><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
    <section className={`${styles.section} shell`}><div className="section-kicker">Vendor categories</div><div className={styles.heading}><h2>Technology services procurement can map clearly.</h2><p>Categories are written in common procurement language so UNIRAS can be classified consistently in vendor systems.</p></div><div className={styles.tags}>{categories.map((item)=><span key={item}>{item}</span>)}</div></section>
    ...
    <section className={`${styles.section} ${styles.note}`}><div className={`shell ${styles.noteInner}`}><div><div className="section-kicker">Procurement contact</div><h2>Need UNIRAS registered in your vendor system?</h2><p>Send the registration form, document checklist or procurement requirements. We will respond with the relevant corporate information through the appropriate channel.</p></div><a className="button primary" href="mailto:uniras.capital@gmail.com?subject=Vendor%20Registration%20-%20UNIRAS%20SDN.%20BHD.">Request vendor documents</a></div></section>
